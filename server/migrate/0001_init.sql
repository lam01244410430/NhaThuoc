...
export const shopDispatchAddresses = sqliteTable(
  "shop_dispatch_addresses",
  {
    dispatchAddressId: integer("dispatch_address_id").primaryKey({ autoIncrement: true }),
    shopId: integer("shop_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("uq_shop_dispatch_one_default")
      .on(table.shopId)
      .where(sql`${table.isDefault} = 1`),
    check("ck_dispatch_contact_name", sql`length(trim(${table.contactName})) > 0`),
    check("ck_dispatch_phone", sql`length(trim(${table.phone})) > 0`),
    check("ck_dispatch_province", sql`length(trim(${table.province})) > 0`),
    check("ck_dispatch_address", sql`length(trim(${table.addressDetail})) > 0`),
    check("ck_dispatch_default", sql`${table.isDefault} IN (0, 1)`),
    check("ck_dispatch_status", sql`${table.status} IN ('active', 'inactive')`),
    check(
      "ck_dispatch_coordinates",
      sql`(
    ...
    index("idx_dispatch_shop_status").on(table.shopId, table.status),
    index("idx_dispatch_location").on(table.province, table.district),
  ],
);
...
export const warehouses = sqliteTable(
  ...
  (table) => [
    ...
    check("ck_warehouse_latitude", sql`${table.latitude} BETWEEN -90 AND 90`),
    check("ck_warehouse_longitude", sql`${table.longitude} BETWEEN -180 AND 180`),
...
export const inboundShipments = sqliteTable(
  "inbound_shipments",
  {
    ...
    dispatchAddressId: integer("dispatch_address_id")
      .notNull()
      .references(() => shopDispatchAddresses.dispatchAddressId, {
        onDelete: "restrict",
        onUpdate: "cascade",
      }),
...
export type ShopDispatchAddress = typeof shopDispatchAddresses.$inferSelect;
export type NewShopDispatchAddress = typeof shopDispatchAddresses.$inferInsert;
